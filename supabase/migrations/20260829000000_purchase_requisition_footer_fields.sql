-- Adds the sign-off section from the paper requisition form's footer
-- ("Requisitioned: (Name/Rank)" and "Captain / Chief Engineer:"), surfaced
-- while wiring up import of the real VBA-driven Stores requisition template.
-- Both are plain nullable text, shown for every category (not just Stores),
-- following the same direct-column-on-purchase_requisitions precedent as
-- remarks/required_port rather than a new child table.
--
-- Note: requisitioned_by (a person's name/rank) is unrelated to the existing
-- requested_by (a date, "required by") despite the similar name — don't
-- conflate the two.
alter table public.purchase_requisitions
  add column requisitioned_by text,
  add column captain_chief_engineer text;

-- Adding parameters changes the argument-type list, so per the existing
-- precedent (20260825030000_purchase_requisition_new_fields.sql) this is a
-- genuine overload, not an in-place replace: the old 18-arg signature is
-- dropped first, then the 20-arg version is created and re-granted (grants
...
create function public.create_purchase_requisition(
  ...
  p_requisitioned_by text,
  p_captain_chief_engineer text,
...
begin
  ...
  insert into public.purchase_requisitions
    (priority, requested_by, required_port, remarks, requisition_number, requisition_date, title,
     ...
     equipment_specifications, equipment_other_details, requisitioned_by, captain_chief_engineer, created_by)
  values
    (p_priority, p_requested_by, p_required_port, p_remarks, p_requisition_number, p_requisition_date,
     ...
     p_equipment_model, p_equipment_specifications, p_equipment_other_details, p_requisitioned_by,
     p_captain_chief_engineer, v_uid)
...
revoke all on function public.create_purchase_requisition
  (public.pr_priority, date, text, text, text, date, text, text, text, text, text, text, text, text,
   text, text, jsonb, jsonb, jsonb, jsonb) from public;
grant execute on function public.create_purchase_requisition
  (public.pr_priority, date, text, text, text, date, text, text, text, text, text, text, text, text,
   text, text, jsonb, jsonb, jsonb, jsonb) to authenticated;
...
begin
  ...
  set priority = p_priority,
      ...
      requisitioned_by = p_requisitioned_by,
      captain_chief_engineer = p_captain_chief_engineer
  where purchase_requisitions.id = p_id
...
revoke all on function public.update_purchase_requisition
  (uuid, public.pr_priority, date, text, text, text, date, text, text, text, text, text, text, text,
   text, text, text, jsonb, jsonb, jsonb, jsonb) from public;
grant execute on function public.update_purchase_requisition
  (uuid, public.pr_priority, date, text, text, text, date, text, text, text, text, text, text, text,
   text, text, text, jsonb, jsonb, jsonb, jsonb) to authenticated;
...
begin
  ...
  insert into public.purchase_requisitions
    (priority, status, requested_by, required_port, remarks, requisition_number, requisition_date,
     ...
     equipment_specifications, equipment_other_details, requisitioned_by, captain_chief_engineer,
     created_by)
  values
    (v_src.priority, 'pending_rfq', v_src.requested_by, v_src.required_port, v_src.remarks,
     ...
     v_src.equipment_specifications, v_src.equipment_other_details, v_src.requisitioned_by,
     v_src.captain_chief_engineer, v_uid)
