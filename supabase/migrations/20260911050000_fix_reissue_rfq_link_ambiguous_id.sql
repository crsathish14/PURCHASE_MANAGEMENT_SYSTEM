-- Fixes a bug in reissue_rfq_link (20260911040000), caught during manual
-- verification: `returns table (id uuid, access_token text)` implicitly
-- declares `id` as a PL/pgSQL variable visible throughout the function body,
-- so the expire-old-link UPDATE's bare `where id = p_link_id` was ambiguous
-- (42702) against purchase_requisition_rfq_links.id — the exact pitfall
-- issue_rfq_link's own status-flip UPDATE already fully qualifies its
-- columns to avoid. Signature/return type are unchanged, so a plain
-- `create or replace` (no drop) is enough.
...
begin
  ...
  -- stays exactly as expired as it was. Columns fully qualified — id/
  -- access_token/expires_at are all also RETURNS TABLE / declared names,
  -- and would otherwise resolve to those instead of the table's own columns.
  update public.purchase_requisition_rfq_links
  set expires_at = now()
  where purchase_requisition_rfq_links.id = p_link_id
    and purchase_requisition_rfq_links.expires_at > now();
