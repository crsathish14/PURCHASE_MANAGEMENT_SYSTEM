...
export function ServiceQuoteComparisonCard({ vendor, pr, isAwarded, canAward, onAward }: ServiceQuoteComparisonCardProps) {
  return (
            ...
            <tbody>
              {vendor.lineItems.map((item, index) => (
                <tr key={item.lineItemId} className="border-b border-line last:border-b-0">
                  ...
                  <td className={cellClass}>
                    <Input disabled value={displayValue(item.sparesConsumablesIncluded)} />
                  </td>
                  <td className={cellClass}>
