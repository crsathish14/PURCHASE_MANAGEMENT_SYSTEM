...
'
' Built for the "Service Requisition Form" template - the third,
' fully independent form in this project (after Stores and Spares).
' Same proven architecture, entirely separate constants/worksheet,
' no shared state or dependency on either other form.
'===================================================================

' --- Sheet / protection ---
Public Const SHEET_NAME As String = "Service Requisition Form"
...
' Row 32 is fixed forever: nothing is ever inserted above it, so it
' never shifts, unlike every other anchor in this workbook. (This
' form's column headers span 3 rows, not 2 like Spares/Stores, which
' is why the first data row lands one row later than you might expect
' by comparison - always verified directly against the template
' rather than assumed from the other forms.)
Public Const LINEITEM_FIRST_ROW As Long = 32
