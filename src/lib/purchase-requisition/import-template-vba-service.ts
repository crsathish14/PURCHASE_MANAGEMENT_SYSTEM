...
// The third, fully independent VBA workbook/project (after Stores and
// Spares — see templates/vba-source/service/Service_Requisition_Setup_Guide.md).
// Its line-items table is simpler than Stores/Spares: no Qty, Part No., UOM,
// or ROB column at all — a service isn't quantified the way a stores/spares
// part is (line-items-field.tsx's own isService flag already hides the main
// Qty field for this category). Its own port field is labeled "Service
// Port" rather than "Supply Port", still mapped to the same `requiredPort`
// form field. Everything generic (merge-aware label lookup, dynamic table
// sizing, photo-anchor extraction) lives in import-template-vba-shared.ts.
// Exported so export-template-vba-service.ts (the write-side mirror of this
...
export const LABEL = {
  ...
  title: "title",
  availableOnboard: "spares / consumables required - available onboard (y/ n)",
  remarks: "remarks",
...
export function findLineItemColumns(sheet: ExcelJS.Worksheet, headerRow: number): Map<string, number> {
  ...
  sheet.getRow(headerRow).eachCell((cell, colNumber) => {
    ...
    else if (norm === LABEL.availableOnboard) key = PR_LINE_ITEM_PRESET_COLUMN.AVAILABLE_ONBOARD;
...
export function parseServiceVbaRequisitionTemplate(
  ...
  dropdowns.category = PR_CATEGORY.SERVICE;

  const columns = getPresetColumnsForCategory(PR_CATEGORY.SERVICE);
  ...
  if (headerRow !== null) {
    ...
    rows.forEach((row, index) => {
      // row.qty is always "" here — Service's columnMap never has a "qty"
      // entry (no Qty column exists on this template), matching
      // line-items-field.tsx's own isService flag hiding the Qty field
      ...
      // matching Stores/Spares) — every image found for this Sl No. is
      // imported.
      const blobs = images
  ...
  const values: Partial<CreateRequisitionFormValues> = {
    ...
    dropdowns,
    requestedBy: "",
    requiredPort: readLabeledValue(sheet, mergeMap, findLabelCell(sheet, LABEL.servicePort)),
